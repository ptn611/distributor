...
pub fn process_find_airdrop_version(args: &Args, new_distributor_args: &FindAirdropVersionArgs) {
    ...
    println!(
        "Starting to find airdrop version for mint: {}, starting from: {}",
        args.mint, start_version_check
    );
    ...
    while current_version < 1000000000000000000 {
        ...
        match rpc_client.get_account_data(&distributor_pubkey) {
            Ok(_) => {
                println!(
                    "Airdrop version {} exists, account: {}",
                    current_version, distributor_pubkey
                );
            }
            Err(e) => {
                if e.to_string().contains("AccountNotFound") {
                    println!(
                        "Airdrop version does not exist: {} <- next airdrop_version",
                        current_version
                    );
                    break;
                } else {
